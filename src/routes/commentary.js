...
commentaryRouter.get("/", async(req,res) => {
    ...
    if(!paramsResult.success){
        return res.status(400).json({ error: "Invalid query parameters.", details: paramsResult.error.issues }); 
    }
    ...
    try {
        ...
        console.error("Failed to retrieve commentary list:", error);
        res.status(500).json({ error: 'Failed to retrieve commentary list.'});
    }
})